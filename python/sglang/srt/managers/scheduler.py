...
class Scheduler(
    ...
    def set_forward_stream(self, stream):
        self.kernel_launch_stream = stream
    ...
    def run_batch(
        ...
        if self.is_generation:
            if self.spec_algorithm.is_none():
                ...
                if self.pp_group.is_last_rank:
                    with torch.cuda.stream(self.kernel_launch_stream):
                        logging.debug(f"thread: {threading.current_thread().name} forward stream: {self.kernel_launch_stream}")
